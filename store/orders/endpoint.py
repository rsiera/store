...
class Endpoint:
    ...
    def _call_method(self, method_name, **kwargs):
        ...
        with timer(self._log(method_name)):
            try:
                ...
                raise ApiConnectionError('Error calling Store API: {}'.format(e))
            else:
    ...
    def _get_method(self, method_name):
        if method_name not in self._methods:
            msg = '{} not allowed for endpoint {}'.format(method_name, self.url)
            raise self.MethodNotAllowed(msg)
        return getattr(self._requests, method_name)

    def _log(self, method_name):
        msg = 'store api | {} | {} | {}'
        return msg.format(method_name.upper(), self.auth[0], self.url)
